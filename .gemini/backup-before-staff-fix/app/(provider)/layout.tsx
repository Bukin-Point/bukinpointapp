...
export default async function ProviderLayout({
  ...
  if (!finalSubdomainProviderId && hostHeader) {
    ...
    // Handle: business-two.bukinpoint.test:3000 -> business-two
    const hostWithoutPort = hostHeader.split(':')[0] // Remove port
    const domainParts = hostWithoutPort.split('.')
    
    // #region agent log
    fetch('http://127.0.0.1:7246/ingest/55297bb7-6ff5-481e-a112-b56b6ed47700',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'layout.tsx:32',message:'Fallback: Attempting subdomain extraction',data:{hostHeader,hostWithoutPort,domainParts,domainPartsLength:domainParts.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    
    if (domainParts.length >= 3 && (domainParts.includes('test') || domainParts.includes('localhost'))) {
      const extractedSubdomain = domainParts[0].toLowerCase()
