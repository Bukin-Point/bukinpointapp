...
export async function getFilteredBookings(params: z.infer<typeof filteredBookingsSchema>) {
  ...
  if (!canViewAll && 'userProvider' in accessContext) {
    ...
    bookingWhere.userProviderId = validated.staffId
  }
...
export async function createManualBooking(data: z.infer<typeof createManualBookingSchema>) {
  try {
    ...
    // Check if user provider can provide this service
    const userProviderService = await prisma.userProviderService.findFirst({
      where: {
        userProviderId: validated.staffId,
        serviceId: validated.serviceId,
      },
    })

    if (!userProviderService && !validated.overrideAvailability) {
    ...
    if (booking.customerEmail) {
      emailPromises.push(
        sendBookingConfirmationEmail({
          ...
          startTime: booking.startTime,
          endTime: booking.endTime,
          staffName: booking.userProvider.user.name || booking.userProvider.user.email,
...
export async function rescheduleBooking(data: z.infer<typeof rescheduleBookingSchema>) {
  try {
    ...
    const conflictingBooking = await prisma.booking.findFirst({
      where: {
        userProviderId: booking.userProviderId,
