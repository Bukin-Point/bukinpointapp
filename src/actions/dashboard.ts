...
export async function getDashboardStats(providerId: string, staffUserId?: string) {
  try {
    ...
    // If staffUserId is provided, filter bookings to only those assigned to this user provider
    if (staffUserId) {
      ...
      if (userProvider) {
        ...
        // If user provider not found, return empty stats
        return {
          success: true,
          stats: {
            totalBookings: 0,
    ...
    if (!staffUserId) {
      ...
      activeStaff = await prisma.userProvider.count({
        where: {
          providerId,
          isActive: true,
        },
